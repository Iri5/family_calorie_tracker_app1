  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-base font-semibold text-foreground">Члены семьи</h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            {user.familyMembers.length} участник
          </p>
        </div>
      ...
      {user.familyMembers.length === 0 && (
        <div className="bg-card rounded-xl border border-border p-12 text-center">
          ...
          />
          <p className="font-semibold text-foreground">Пока нет членов семьи</p>
          <p className="text-sm text-muted-foreground mt-1">
            Добавьте членов семьи, чтобы рассчитать их нормы КБЖУ.
          </p>
        </div>
      )}
            ...
            <div key={m.id} className="bg-card rounded-xl border border-border p-4 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground mt-0.5">
                  {m.age} лет · {m.weight} кг · {m.height} см · {m.sex}
                </div>
                <div className="text-xs text-muted-foreground mt-0.5">
                  Норма {tdee} ккал · Цель {cg} ккал
                  {m.customCalorieGoal && <span className="text-primary ml-1">(пользовательская)</span>}
                </div>
                <div className="flex gap-2.5 mt-2 text-xs tabular-nums">
                  <span className="text-blue-600">Б {mg.protein}г</span>
                  <span className="text-amber-600">Ж {mg.fat}г</span>
                  <span className="text-orange-600">У {mg.carbs}г</span>
                </div>
              </div>
      ...
      <Modal
        ...
        title={editId ? "Редактировать" : "Добавить"}
        footer={
          <>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <Field label="Имя">
            <TInput value={form.name} onChange={v => setF("name", v)} placeholder="Имя участника" />
          </Field>
          ...
          <Field
            label="Персональная цель (ккал)"
            hint={tdeePreview ? `Рассчитанная норма: ${tdeePreview} ккал/день` : undefined}
          >
            <NInput
              value={form.customCalorieGoal}
              onChange={v => setF("customCalorieGoal", v)}
              placeholder={tdeePreview ? String(tdeePreview) : "Оставьте пустым для расчёта"}
              min={500}
            />
          </Field>
        </div>
