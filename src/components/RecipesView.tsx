...
export function RecipesView({ data, onUpdateData }: RecipesViewProps) {
  ...
  const preview = useMemo(() => {
    ...
    const tmp: Recipe = { id: "", name: "", description: "", ingredients: form.ingredients };
    return {
  ...
  const openEdit = (r: Recipe) => {
    setForm({ name: r.name, description: r.description, ingredients: [...r.ingredients] });
    setEditId(r.id);
    setShowForm(true);
  };

  const addIngredient = (productId: string) => {
    setForm(f => ({ ...f, ingredients: [...f.ingredients, { productId, grams: 100 }] }));
    setIngSearch("");
              ...
              <div key={r.id} className="bg-card rounded-xl border border-border p-4">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex gap-1 shrink-0 ml-3">
                    ...
                    <button
                      onClick={() =>
                        onUpdateData({ ...data, recipes: data.recipes.filter(x => x.id !== r.id) })
                      }
                ...
                return (
                  <div key={ing.productId} className="flex items-center gap-2 bg-muted/40 rounded-lg px-3 py-2">
                    ...
                    <input
                      type="number"
                      value={ing.grams}
                      onChange={e => updateGrams(ing.productId, parseFloat(e.target.value) || 0)}
                      min={1}
